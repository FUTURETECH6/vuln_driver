...
static long do_ioctl(struct file *filp, unsigned int cmd, unsigned long args) {
    ...
    switch (cmd) {
        case DRIVER_TEST: printk(KERN_WARNING "[x] Talking to device [x]\n"); break;
        case BUFFER_OVERFLOW: buffer_overflow((char *)args); break;
        case NULL_POINTER_DEREF: null_pointer_deref(args); break;
        case ALLOC_UAF_OBJ: alloc_uaf_obj(args); break;
        case USE_UAF_OBJ: use_uaf_obj(); break;
        case ALLOC_K_OBJ: alloc_k_obj((k_object *)args); break;
        case FREE_UAF_OBJ: free_uaf_obj(); break;
 ...
 */
static struct miscdevice vuln_device = {MISC_DYNAMIC_MINOR, DEVICE_NAME, &vuln_ops};
